...
def to_megatron(megatron_states: Dict[str, Any], huggingface_states: Dict[str, Any]):
    # step 1: create a config file containing the model architecture
    with torch.inference_mode():
        states_dicts = megatron_states["model"]["language_model"]
        states_dicts["embedding"]["word_embeddings"]["weight"] = huggingface_states["model.embed_tokens.weight"].contiguous() 
        # new_state_dict["model.embed_tokens.weight"] = states_dicts["embedding"]["word_embeddings"]["weight"].contiguous()
        
        states_dicts["embedding"]["word_embeddings"]["weight"] = huggingface_states["lm_head.weight"].contiguous()
        ...
        for i in range(hf_config.num_hidden_layers):
            ...
            new_state_dict[f"model.layers.{i}.self_attn.q_proj.weight"] = q.transpose(0, 1).contiguous()
            new_state_dict[f"model.layers.{i}.self_attn.k_proj.weight"] = k.transpose(0, 1).contiguous()
            new_state_dict[f"model.layers.{i}.self_attn.v_proj.weight"] = v.transpose(0, 1).contiguous()
